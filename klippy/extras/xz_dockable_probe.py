...
class DockableProbe:
    def __init__(self, config):
        ...
        self.endstop_offset = config.getfloat('endstop_offset', 0)
        ...
        try:
            ...
            raise config.error("Unable to parse home_xy_position in %s, %s"
                               % (config.get_name(),e,))
	...
	x_off,y_off,z_off = self.printer.lookup_object('probe').get_offsets()
        self.home_x_pos -= x_off
        self.home_y_pos -= y_off
    ...
    def _endstop_descend(self):
        ...
        ## Slight adjustment given offset. 
        ## homing_move sets toolhead position=position_endstop, below is workaround to go above/lower to reach dock.
        cpos = self.toolhead.get_position()
        cpos[2] -= self.endstop_offset
        self.toolhead.set_position(cpos)
        self.toolhead.manual_move([None, None, cpos[2] + self.endstop_offset], self.z_speed)
